...
root.title("Car and Road Damage Detection")
...
#
lbl = tk.Label(root, text="Welcome to Car Damage Detection System", font=('times', 25,' bold underline '), width=70,height=1,bg="black",fg="white")
lbl.place(x=0, y=0)
...
def update_cal(str_T):
    #clear_img()
    result_label = tk.Label(root, text=str_T, width=40, font=("bold", 25), bg='skyblue', fg='black')
...
def test_model_proc(fn):
    ...
    if fn!="":
        ...
        if yoga == 0:
            Cd="Car Damage Detected"
            
        if yoga == 1:
            Cd="Car Not Damage Detected"
      
            
        A=Cd
        return A
    img = open(fn)    
    myfile = open("D:/23 Protech/100% code/Car Damage Detection and Road Pothole/Car and Road damage detection 100% code/Car and Road damage detection 100% code/testing")
    if img == myfile:
          print(myfile)
...
def openimage():
    ...
    global fn
    print(fn)
    fileName = askopenfilename(initialdir='D:/23 Protech/100% code/Car Damage Detection and Road Pothole/Car and Road damage detection 100% code/Car and Road damage detection 100% code/testing', title='Select image for Aanalysis ',
                               filetypes=[("all files", "*.*")])
    IMAGE_SIZE=200
    imgpath = fileName
    fn = fileName
...
button2 = tk.Button(frame_alpr, text="CNN Algorithm", command=test_model, width=15, height=1, font=('times', 15, ' bold '),bg="white",fg="black")
button2.place(x=10, y=190)

button2 = tk.Button(frame_alpr, text="EXIT", command=close, width=15, height=1, font=('times', 15, ' bold '),bg="white",fg="red")
button2.place(x=10, y=260)


exit = tk.Button(root, text="Next", command=window, width=15, height=1, font=('times', 15, ' bold '),bg="red",fg="black")
exit.place(x=1000, y=650)
